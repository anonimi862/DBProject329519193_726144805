...
# דוח פרויקט - שלב ה': ממשק גרפי למערכת ניהול מחסנים

## תיאור כללי
בשלב זה פותח ממשק גרפי מלא למערכת ניהול המחסנים והלוגיסטיקה. הממשק פותח בשפת Python תוך שימוש בספריית Tkinter ומאפשר ביצוע כל פעולות ה-CRUD, הרצת שאילתות מורכבות והפעלת פונקציות ופרוצדורות.
...
3. **הכנת בסיס הנתונים:**
- וודאו שבסיס הנתונים מוכן עם כל הטבלאות, הפונקציות והפרוצדורות משלבים קודמים
- שימו לב לשם בסיס הנתונים, שם המשתמש והסיסמה
...
2. **כניסה למערכת:**
- Host: localhost (ברירת מחדל)
- Database: שם בסיס הנתונים שלכם
- Username: שם המשתמש
- Password: הסיסמה
...
### שפת תכנות
- **Python 3.9** - נבחרה בשל:
  - תמיכה מעולה ב-PostgreSQL דרך psycopg2
  - ספריית Tkinter מובנית לממשק גרפי
  - קלות פיתוח ותחזוקה
  - תאימות מלאה לכל מערכות ההפעלה

### ספריות
- **Tkinter** - ממשק גרפי
  - מובנית ב-Python, אין צורך בהתקנות נוספות
  - תמיכה מלאה ב-widgets מתקדמים
  - יכולות עיצוב גמישות

- **psycopg2** - התממשקות ל-PostgreSQL
  - ספרייה בוגרת ויציבה
  - תמיכה מלאה בכל תכונות PostgreSQL
  - ביצועים מעולים

### ארכיטקטורת המערכת
- **MVC Pattern** - הפרדה בין לוגיקה, נתונים ותצוגה
- **Modular Design** - כל מסך בקובץ נפרד
- **Event-Driven** - תגובה לאירועי משתמש
- **Connection Pooling** - שימוש יעיל בחיבורי מסד נתונים
...
**תיאור:** מסך הכניסה הראשוני למערכת
- שדות להזנת פרטי חיבור לבסיס הנתונים
- בדיקת תקינות החיבור
- הודעות שגיאה ברורות במקרה של כשל
...
**תיאור:** מסך התפריט הראשי עם גישה לכל תת-המערכות
- תצוגת שם המשתמש המחובר
...
### 3. ניהול מחסנים (Warehouse Management)
![Warehouse Management](screenshots/warehouse_management.png)

**תיאור:** מסך מקיף לניהול מחסנים עם שלוש לשוניות:

**לשונית מחסנים:**
- תצוגת כל המחסנים עם מידע על קיבולת וניצול
- צביעת שורות לפי רמת הניצול (אדום = מעל 80%)
- CRUD מלא על מחסנים

**לשונית מלאי:**
- בחירת מחסן וצפייה במלאי
- עדכון כמויות
- הוספה/הסרה של חלקים

**לשונית עובדים:**
- צפייה בעובדים לפי מחסן
- הקצאת עובדים למחסנים
- ניהול צוותי עבודה

**פעולות CRUD:**
- **Create:** הוספת מחסן חדש עם כל הפרטים
- **Read:** צפייה במידע מפורט כולל חישובי ניצול
- **Update:** עריכת פרטי מחסן, מלאי ועובדים
- **Delete:** מחיקת מחסן (עם בדיקת תלויות)

### 4. ניהול חלקים (Parts Management)
![Parts Management](screenshots/parts_management.png)

**תיאור:** מסך לניהול מלאי החלקים והספקים
- חיפוש חלקים בזמן אמת
- תצוגת מחירים מינימום/מקסימום
- ניהול ספקים לכל חלק

**פעולות CRUD:**
- **Create:** הוספת חלק חדש למערכת
- **Read:** צפייה בכל החלקים עם סיכום מידע
- **Update:** עדכון פרטי חלק ומחירי ספקים
- **Delete:** מחיקת חלק (כולל כל הקשרים)

**ניהול ספקים:**
- הוספת/הסרת ספקים לחלק
- עדכון מחירים וכמויות
- השוואת מחירים בין ספקים

### 5. ניהול הזמנות (Orders Management)
![Orders Management](screenshots/orders_management.png)

**תיאור:** מסך מקיף לניהול הזמנות
- תצוגת סטטוס בצבעים (אדום = באיחור)
- סינון לפי סטטוס
- חישוב עלויות אוטומטי

**פעולות CRUD:**
- **Create:** יצירת הזמנה חדשה עם בחירת ספק אופטימלי
- **Read:** צפייה בכל ההזמנות עם סינון
- **Update:** עדכון פרטי הזמנה
- **Delete:** מחיקת הזמנה

**פעולות מיוחדות:**
- סימון הזמנה כ"הגיעה" - מפעיל טריגר לעדכון מלאי
- חישוב עלות כוללת בזמן אמת
- התראות על הזמנות באיחור

### 6. הרצת שאילתות (Queries Screen)
![Queries Screen](screenshots/queries_screen.png)

**תיאור:** מסך להרצת שאילתות מוכנות מראש
- בחירת שאילתא מרשימה
- תצוגת תוצאות בטבלה
- יצוא תוצאות

**שאילתות זמינות:**
1. **מחסנים עם הכי הרבה חלקים** - ניתוח מקיף של מחסנים
2. **עובדים הזקוקים להכשרה** - רשימת עובדים שלא עברו הכשרה שנתית
3. **ניתוח הזמנות לפי חודש** - סטטיסטיקות הזמנות
4. **התראת מלאי נמוך** - חלקים עם מלאי מתחת ל-50
5. **ניתוח חלקים פופולריים** - החלקים המוזמנים ביותר

### 7. הרצת פרוצדורות ופונקציות (Procedures Screen)
![Procedures Screen](screenshots/procedures_screen.png)

**תיאור:** מסך להפעלת פונקציות ופרוצדורות מתקדמות

**לשונית פונקציות:**
1. **AnalyzeWarehouseOperations** - ניתוח מקיף של מחסן
   - בחירת מחסן
   - הצגת סטטיסטיקות מפורטות
   
2. **GetMaintenanceSchedule** - לוח זמנים לתחזוקה
   - הגדרת טווח זמן
   - סינון לפי מיקום
   - תצוגת רכבות ומטוסים הזקוקים לתחזוקה

**לשונית פרוצדורות (המשך):**
1. **ProcessPendingOrders** - עיבוד הזמנות ממתינות
   - בחירת תאריך ומספר הזמנות
   - תצוגת לוג מפורט של העיבוד
   - עדכון אוטומטי של מלאי המחסנים

2. **OptimizeFleetAssignment** - אופטימיזציה של הקצאת צי
   - בחירת מפעיל או כל המפעילים
   - אפשרות לאיזון מחדש של הקצאות
   - דוח מפורט על השינויים שבוצעו

## תכונות מיוחדות

### 1. עדכונים בזמן אמת
- רענון אוטומטי של נתונים לאחר כל פעולה
- סנכרון בין מסכים שונים
- התראות על שינויים קריטיים

### 2. בדיקות ואימותים
- בדיקת תקינות קלט לפני ביצוע פעולות
- אימות קשרי גומלין בין טבלאות
- הודעות שגיאה ברורות ומפורטות

### 3. טריגרים אוטומטיים
- עדכון מלאי בעת סימון הזמנה כ"הגיעה"
- חישוב תאריכי תחזוקה אוטומטיים
- התראות על חריגות

### 4. ממשק משתמש ידידותי
- עיצוב נקי ואינטואיטיבי
- קיצורי מקלדת (Enter לאישור, Escape לביטול)
- חיפוש וסינון בזמן אמת
- לחיצה כפולה לעריכה

## צילומי מסך נוספים

### דוגמה לפעולת CRUD - הוספת מחסן
![Add Warehouse](screenshots/add_warehouse.png)

**תהליך:**
1. לחיצה על "Add Warehouse"
2. מילוי הטופס עם פרטי המחסן
3. אימות נתונים
4. שמירה והודעת הצלחה

### דוגמה לשאילתא - מחסנים עם הכי הרבה חלקים
![Query Results](screenshots/query_results.png)

**תוצאות:**
- תצוגה טבלאית מסודרת
- אפשרות למיון לפי עמודות
- סיכום בתחתית

### דוגמה לפרוצדורה - עיבוד הזמנות
![Procedure Output](screenshots/procedure_output.png)

**פלט:**
- לוג מפורט של הפעולות
- סטטיסטיקות ביצוע
- הודעות על שגיאות אם היו

### ניהול קשרים בין טבלאות - הקצאת ספקים לחלקים
![Supplier Management](screenshots/supplier_management.png)

**יכולות:**
- ניהול קשרי many-to-many
- עדכון מחירים וכמויות
- מחיקת קשרים
...
### 1. ניהול חיבורים
- חיבור יחיד לכל session
- סגירת חיבור בעת logout
- טיפול בניתוקים

### 2. טעינת נתונים
- טעינה מושהית (lazy loading)
- מטמון לנתונים סטטיים
- רענון רק בעת הצורך

### 3. טיפול בשגיאות
- Try-catch מקיף
- Rollback אוטומטי בעת כשל
- לוגים מפורטים

## בעיות ידועות ופתרונות

### 1. בעיה: חיבור נכשל למסד הנתונים
**פתרון:** 
- ודאו ש-PostgreSQL פועל
- בדקו את פרטי החיבור
- ודאו שהמשתמש קיים ויש לו הרשאות

### 2. בעיה: פונקציות/פרוצדורות לא נמצאות
**פתרון:**
- הריצו את הסקריפטים משלב ד'
- ודאו שהם נוצרו באותו schema

### 3. בעיה: עדכון מלאי לא עובד
**פתרון:**
- בדקו שהטריגרים פעילים
- ודאו שיש הרשאות מתאימות
...
4. **אינטגרציות**
   - ייבוא נתונים מ-CSV
   - API לגישה חיצונית
   - התממשקות למערכות ERP

## סיכום

המערכת שפותחה מספקת ממשק גרפי מלא ומקיף לניהול מחסנים ולוגיסטיקה. היא כוללת:

- **5+ מסכים** פונקציונליים
- **CRUD מלא** על 3 טבלאות מרכזיות וטבלת קישור
- **הרצת שאילתות** מורכבות
- **הפעלת פונקציות ופרוצדורות** מתקדמות
- **ממשק ידידותי** ואינטואיטיבי
- **טיפול מקיף בשגיאות**

המערכת מוכנה לשימוש מידי ומספקת את כל הכלים הנדרשים לניהול יעיל של מחסנים, מלאי, הזמנות ועובדים.

## קבצים בפרויקט

```
שלב_ה/
├── main.py                          # התוכנית הראשית
├── screens/                         # תיקיית המסכים
│   ├── __init__.py
│   ├── login_screen.py             # מסך כניסה
│   ├── main_menu.py                # תפריט ראשי
│   ├── warehouse_management.py     # ניהול מחסנים
│   ├── parts_management.py         # ניהול חלקים
│   ├── orders_management.py        # ניהול הזמנות
│   ├── queries_screen.py           # הרצת שאילתות
│   └── procedures_screen.py        # הרצת פרוצדורות
...
└── screenshots/                    # צילומי מסך
    ...
    ├── parts_management.png
    ├── orders_management.png
    ├── queries_screen.png
    ├── procedures_screen.png
    └── [צילומי מסך נוספים]
```
